The project targets the gap between packet-level validity and process-level safety. ICS-Guard is built around a simulated water-treatment process and asks whether each incoming control command is safe in its current context. The implementation is advisory: it identifies risk, explains why, and asks for engineer review rather than automatically changing plant equipment.
